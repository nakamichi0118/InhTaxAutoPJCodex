...
- **JON API統合** - 不動産タブでJON APIを利用した情報自動取得
  - 位置特定API（住所→座標変換）
  - 登記情報取得（PDF形式）
  - 路線価図取得（画像形式）
  - Googleマップリンク自動生成
...
- **JON API関連バックエンド**
  - `backend/app/jon/` パッケージ新規作成
  - `client.py` - JON APIクライアント（認証・リトライ処理）
  - `router.py` - FastAPIルーター（/api/jon/*）
  - `models.py` - Pydanticモデル
  - `rosenka_scraper.py` - 国税庁路線価図URLスクレイパー（403制限のため無効化）

- **不動産タブパスワードロック**
  - パスワード「IUProperty7520」で保護
  - デフォルトで通帳・取引履歴タブを表示
  - ロックアイコン（🔒）表示

- **取得オプションUI**
  - 不動産ごとに取得項目をチェックボックスで選択可能
  - 地図チェックボックスの自動連動（登記・路線価・図面選択時に地図を自動ON）

- **VBA印刷機能** - `transaction_import.bas`
  - 印刷機能の追加
  - 用途欄転記機能
...
- **取得結果の表示形式変更**
  - インライン表示から表形式（テーブル）に変更
  - 状態・種別・所在地・地図・登記・路線価の各列を整列

- **路線価リンクの改善**
  - 路線価図がない場合は「倍率表」リンクを表示
  - 都道府県に応じた国税庁サイトへのリンク
...
- Geminiモデル切り替え時の問題を修正
- 不動産タブのデータ表示とJON API連携の問題を修正
- フィールドマッピング修正（ネスト構造対応）

### Documentation（ドキュメント）

- `webapp/help.html` に不動産タブの使い方を追加
- FAQ（路線価図・登記簿・位置特定）を追加

### Technical Details

- `backend/app/config.py` - JON API認証情報設定追加
- `backend/app/main.py` - JON APIルーター登録
- `backend/requirements.txt` - httpx, beautifulsoup4 追加
- `webapp/index.html` - JON API UI、パスワードロック、テーブル表示
...
| バージョン | リリース日 | 主要機能 |
|-----------|-----------|---------|
| 0.11.0 | 2026-01-27 | JON API統合、不動産タブパスワードロック、取得結果テーブル表示 |
